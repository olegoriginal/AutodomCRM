      ...
      <div className="container mx-auto px-4">
        ...
        <div className="py-3 px-4 my-3 rounded-lg shadow bg-white">
          ...
          <div>
            <h3>Год:</h3>
            {autopartsList
              .map((it, index) => new Date(it.date).getFullYear())
              ...
              .map((it, index) => (
                <button
                  className={cx(
                    "p-2 shadow text-white hover:text-white rounded-lg my-3 mx-3",
                    {
                      "bg-blue-600 hover:bg-blue-700":
                        date.year === JSON.stringify(it),
          ...
          {date.year ? (
            <div>
              ...
              {autopartsList
                .filter(
                  (item, id) =>
                    JSON.stringify(new Date(item.date).getFullYear()) ===
                    date.year
                )
                .map((it, index) => new Date(it.date).getMonth())
                .filter((item, id, array) => array.indexOf(item) === id)
                .sort(function (a, b) {
                  if (a > b) {
                    return 1
                ...
                .map((it, index) => (
                  <button
                    className={cx(
                      ...
                      {
                        "bg-blue-600 hover:bg-blue-700":
                          date.month === JSON.stringify(it + 1),
                        "bg-gray-500 hover:bg-gray-700":
                          date.month !== JSON.stringify(it + 1),
                      }
                    )}
